        ...
        for th in $thread ; do 
            for nd in $node ; do
                if [[ $md == "clockdown" ]] ; then
                    if [[ $nd == "master" ]] ; then
                        m=`grep "SET" "$1""$md"/thread_"$th"_ops_"$n"/"$nd" | awk '{print NR }' | tail -1`
                        echo "$md ,thread_$th ,ops_$n ,$nd = $m" >>"$1"result_$md 
                    elif [[ $nd == "slave" ]] ; then
                        s_1=`grep -rn "SET" "$1""$md"/thread_"$th"_ops_"$n" | awk '{print NR}' | tail -1`
                        echo "$md ,thread_$th ,ops_$n ,$nd = $s_1" >>"$1"result_$md
                    else
                        s_2=`grep "SET" "$1""$md"/thread_"$th"_ops_"$n"/"$nd" | awk '{print NR}' | tail -1`
